...
# Remove the table of contents and the index, this is specific to 2017
cba_cut <- cba[23:598]

# Key difference so far:
#  1. Page numbers at the top of the page with article/exhibit
#  2. Numbers alternate from right side of page to left side
...
cba_pages_mapped <- map(cba_cut, function(x) {
  ...
  exist_regex <- "(ARTICLE|EXHIBIT) ([A-z]|[A-z]-[0-9])+\n([A-Z]|\\s|[:punct:]|[0-9])*\n"
